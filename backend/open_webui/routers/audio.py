    ...
    if request.app.state.config.STT_ENGINE == '':
        ...
        try:
            for language in languages:
                model = request.app.state.config.STT_MODEL
                if 'cartesia.ai' in request.app.state.config.STT_OPENAI_API_BASE_URL and model == 'ink/ink-whisper':
                    model = 'ink-whisper'
...
async def get_available_voices(request) -> dict:
    ...
    if request.app.state.config.TTS_ENGINE == 'openai':
        ...
        # Cartesia uses a paginated /voices endpoint with a Cartesia-Version header;
        # offload the sync helper to a worker thread so the event loop stays free.
        if 'cartesia.ai' in base_url:
            try:
                available_voices = await asyncio.to_thread(
                    get_cartesia_voices,
