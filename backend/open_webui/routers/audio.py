...
async def _tts_openai(request, payload, file_path, file_body_path, user):
    ...
    # Cartesia uses a Cartesia-Version header (carried in TTS_OPENAI_PARAMS).
    if 'cartesia.ai' in request.app.state.config.TTS_OPENAI_API_BASE_URL:
        ...
        if isinstance(request.app.state.config.TTS_OPENAI_PARAMS, dict):
            cartesia_version = request.app.state.config.TTS_OPENAI_PARAMS.get('cartesia_version')
        if cartesia_version:
    ...
    # Cartesia doesn't implement OpenAI's /audio/speech; use /tts/bytes and
    # reshape the payload into Cartesia's schema.
    if 'cartesia.ai' in tts_base_url:
        ...
        if isinstance(payload.get('output_format'), dict):
            ...
            output_format = request.app.state.config.TTS_OPENAI_PARAMS.get('output_format')
...
async def _transcribe_openai(request, file_path, filename, languages, file_dir, id, user=None):
    ...
    try:
        ...
        for language in languages:
            # Cartesia's ink-whisper is registered as 'ink/ink-whisper' in the
            # model list but the STT API expects the bare 'ink-whisper'.
            model = request.app.state.config.STT_MODEL
            if (
            ...
            payload = {'model': model}
            if language:
                # Cartesia expects a bare language code (e.g. 'en', not 'en-US').
                if (
                    'cartesia.ai' in request.app.state.config.STT_OPENAI_API_BASE_URL
...
@lru_cache
def get_cartesia_voices(api_key: str, base_url: str, cartesia_version: Optional[str]) -> dict:
    """Fetch {voice_id: voice_name} from Cartesia's paginated /voices endpoint.

    Sync (uses requests) so it can be cached; callers offload it to a thread.
    """
    import requests
...
async def get_available_voices(request) -> dict:
    ...
    if engine == 'openai':
        ...
        # Cartesia uses a paginated /voices endpoint with a Cartesia-Version
        # header; offload the sync helper to a thread so the loop stays free.
