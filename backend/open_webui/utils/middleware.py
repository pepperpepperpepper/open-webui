...
from open_webui.utils.models import get_model_meta
...
async def chat_web_search_handler(request: Request, form_data: dict, extra_params: dict, user):
    # Pepper patch: deliberately defer the "Searching the web" emit until we
    # actually have a query to run; otherwise an unrelated chat would briefly
    # flash a "Searching the web → No search query generated" status.
    event_emitter = extra_params['__event_emitter__']
    ...
    def _should_fallback_search(text: str) -> bool:
        ...
        triggers = (
            "today", "latest", "current", "breaking", "news", "as of",
            "right now", "price", "cost", "availability", "schedule",
            "score", "verify", "citation", "cite", "source", "official",
            "look up", "lookup", "search the web", "browse the web",
        )
...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    if (
        ...
        and (
            user.role == 'admin'
            or has_permission(user.id, 'features.web_search', request.app.state.config.USER_PERMISSIONS)
        )
    ):
